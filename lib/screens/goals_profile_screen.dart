...
class _GoalsProfileScreenState extends State<GoalsProfileScreen> {
  ...
  Future<void> _shareProgress() async {
    if (_goals.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No goals to share yet')),
      );
      return;
    }

    final completedGoals = _goals.where((g) => g.isCompleted).length;
    final activeGoals = _goals.where((g) => g.status == 'Active').length;

    // Find the goal with most progress to feature
    final topGoal = _goals.where((g) => !g.isCompleted).isNotEmpty
    ...
    buffer.writeln('🎯 My Spiritual Goals Progress');
    buffer.writeln('');
    buffer.writeln('📊 Overview:');
    buffer.writeln('  ✅ Completed: $completedGoals');
    buffer.writeln('  🔄 Active: $activeGoals');
    buffer.writeln('  📈 Total: ${_goals.length}');

    if (topGoal != null) {
      buffer.writeln('');
      buffer.writeln('🌟 Featured Goal:');
      buffer.writeln('  ${topGoal.title}');
      buffer.writeln(
          '  Progress: ${topGoal.currentCount}/${topGoal.targetCount} (${topGoal.progressPercentage.toStringAsFixed(0)}%)');
      buffer.writeln('  ${topGoal.category} • ${topGoal.frequency}');
    }
    ...
    await Share.share(
      buffer.toString(),
      subject: 'My Spiritual Goals Progress',
    );
  }
  ...
  Future<void> _confirmDeleteGoal(String id) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete Goal'),
        content: const Text(
            'Are you sure you want to delete this spiritual goal? This action cannot be undone.'),
        actions: [
          TextButton(
    ...
    if (confirm == true && mounted) {
      try {
        await LocalStorageService.deleteSpiritualGoal(id);
        await _loadGoals();
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Error deleting goal. Please try again.'),
  ...
  Future<void> _incrementProgress(SpiritualGoal goal, int increment) async {
    ...
    // Check achievements if goal was completed
    if (isCompleted && !goal.isCompleted) {
      final beforeAchievements = await AchievementService.getAchievements();
      ...
      final newlyUnlocked = newAchievements
          .where((a) =>
              a.isUnlocked &&
              !beforeUnlockedIds.contains(a.id) &&
              a.category == 'goal')
          .toList();
