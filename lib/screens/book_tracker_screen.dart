...
class _BookTrackerScreenState extends State<BookTrackerScreen> {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _BookCard(
          ...
          onDelete: () => _confirmDeleteBook(book.id),
        );
      },
    );
  }
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              // Check achievements if book was completed
              if (newPage >= book.totalPages && book.status != 'Completed') {
                ...
                final newAchievements = await AchievementService.checkAndUpdateAchievements();
                final newlyUnlocked = newAchievements.where((a) => 
                  a.isUnlocked && !beforeUnlockedIds.contains(a.id) && a.category == 'book'
                ).toList();
          ...
          children: [
            ...
            ...achievements.map((achievement) => Padding(
              ...
              child: Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                  color: _parseColor(achievement.badgeColor).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Column(
                  children: [
  ...
  Future<void> _confirmDeleteBook(String id) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete Book'),
        content: const Text('Are you sure you want to delete this book from your reading list?'),
        actions: [
          TextButton(
    ...
    if (confirm == true && mounted) {
      try {
        await LocalStorageService.deleteBookReading(id);
        await _loadBooks();
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Error deleting book. Please try again.'),
